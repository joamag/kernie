...
echo ""
echo "=== Step 2: Compile kernel C code ==="
...
gcc -ffreestanding -fno-stack-protector -mno-red-zone -fno-pic \
    -m64 -c -o kernel.o kernel.c

echo ""
echo "=== Step 3: Link kernel into flat binary ==="
echo "  -T kernel.ld    : use our linker script (load at 0x100000)"
echo "  --oformat binary : raw flat binary, no ELF headers"
ld -T kernel.ld -o kernel.bin kernel.o --oformat binary

echo ""
echo "=== Step 4: Combine boot sector + kernel into disk image ==="
echo "  boot.bin   = first 512 bytes (sector 1)"
echo "  kernel.bin = sectors 2+ (loaded by BIOS int 0x13)"
cat boot.bin kernel.bin > os.bin

echo ""
echo "=== Step 5: Pad to floppy size (optional, helps some emulators) ==="
